...
%% 
   if strcmp(plane,'xy') | strcmp(plane,'xz') | strcmp(plane,'yz')
      ...
      plot(Xopt(idx1,:),Xopt(idx2,:),'*b','markersize',markersize); 
      hold on; grid on
      plot([Xopt(idx1,:); PP(idx1,:)],[Xopt(idx2,:); PP(idx2,:)],'b')
      plot(PP(idx1,:),PP(idx2,:),'ok','markersize',markersize); 
      if ~isempty(P0)
         h = plot(P0(idx1,:),P0(idx2,:),'bd','markersize',markersize);    
         set(h,'linewidth',2,'color','r');     
      end
      axis('square'); axis(BoxScale*[0,1,0,1]);
      %axis('square'); axis(BoxScale*[0,1,0,1]);
      pause(0.1); hold off
   elseif strcmp(plane,'xyz')
      plot3(PP(1,:),PP(2,:),PP(3,:),'ok','markersize',markersize); 
      hold on; grid on; 
      plot3(Xopt(1,:),Xopt(2,:),Xopt(3,:),'*b','markersize',markersize); 
      plot3([Xopt(1,:); PP(1,:)],[Xopt(2,:); PP(2,:)],[Xopt(3,:); PP(3,:)],'b');
      if ~isempty(P0)
         h = plot3(P0(1,:),P0(2,:),P0(3,:),'d','markersize',markersize); 
         set(h,'linewidth',3,'color','r');     
      end   
      axis('square'); axis(1*BoxScale*[0,1,0,1,0,1])
      pause(0.1); hold off
   end
%%************************************************************************
